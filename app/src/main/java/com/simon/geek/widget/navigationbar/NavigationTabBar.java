...
package com.simon.geek.widget.navigationbar;
...
import com.simon.geek.widget.navigationbar.behavior.NavigationTabBarBehavior;
...
public class NavigationTabBar extends View implements ViewPager.OnPageChangeListener {
    ...
    // 导航栏图标未被选中
    private final static int DEFAULT_INACTIVE_COLOR = Color.parseColor("#aaaaaa");
    private final static int DEFAULT_ACTIVE_COLOR = Color.WHITE;// 图标颜色
    private final static int DEFAULT_BG_COLOR = Color.parseColor("#ffffff");// 导航栏背景颜色
    ...
    // By default badge bg color is the active model color and badge title color is the model bg
    // color
    ...
    public NavigationTabBar(final Context context, final AttributeSet attrs, final int
            defStyleAttr) {
        ...
        try {
            ...
            setIsBadgeUseTypeface(
                    typedArray.getBoolean(R.styleable.NavigationTabBar_ntb_badge_use_typeface,
                            false)
            );
            ...
            setBadgePosition(
                    typedArray.getInt(
                            R.styleable.NavigationTabBar_ntb_badge_position, BadgePosition
                                    .RIGHT_INDEX
                    )
            );
            setBadgeGravity(
            ...
            setBadgeTitleColor(
                    typedArray.getColor(R.styleable.NavigationTabBar_ntb_badge_title_color,
                            AUTO_COLOR)
            );
            ...
            if (isInEditMode()) {
                ...
                try {
                    ...
                    if (previewColors == null)
                        previewColors = typedArray.getResources().getStringArray(R.array
                                .default_preview);
    ...
    public void setOnTabBarSelectedIndexListener(final OnTabBarSelectedIndexListener
                                                         onTabBarSelectedIndexListener) {
            ...
            mAnimatorListener = new AnimatorListenerAdapter() {
                ...
                public void onAnimationStart(final Animator animation) {
                    if (mOnTabBarSelectedIndexListener != null)
                        mOnTabBarSelectedIndexListener.onStartTabSelected(mModels.get(mIndex),
                                mIndex);
                ...
                public void onAnimationEnd(final Animator animation) {
                    ...
                    if (mOnTabBarSelectedIndexListener != null)
                        mOnTabBarSelectedIndexListener.onEndTabSelected(mModels.get(mIndex),
                                mIndex);
                }
            };
    ...
    private void updateIndicatorPosition(final float fraction) {
        ...
        mPointerLeftTop =
                mStartPointerX + (mResizeInterpolator.getResizeInterpolation(fraction,
                        mIsResizeIn) *
                        (mEndPointerX - mStartPointerX));
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if (width > height) {
            ...
            mIconSize = (int) ((mModelSize > width ? width : mModelSize) *
                    (mIconSizeFraction == AUTO_SCALE ? DEFAULT_ICON_SIZE_FRACTION :
                            mIconSizeFraction));
        }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        for (int i = 0; i < mModels.size(); i++) {
            ...
            if (mIsHorizontalOrientation) {
                ...
                topOffset = (mModelSize * i) + (mModelSize - (float) model.mIcon.getHeight())
                        * 0.5F;
            }
            ...
            final float lastInterpolation = mResizeInterpolator.getResizeInterpolation(mFraction,
                    false);
            ...
            if (mIsSetIndexFromTabBar) {
                if (mIndex == i)
                    ...
                    updateLastModel(
                            ...
                            matrixCenterX, matrixCenterY, matrixLastScale, titleLastScale,
                            titleLastAlpha
                    );
                else
                    updateInactiveModel(
        ...
        for (int i = 0; i < mModels.size(); i++) {
            ...
            if (model.mBadgeFraction == MIN_FRACTION) mBadgePaint.setColor(Color.TRANSPARENT);
                ...
                mBadgePaint.setColor(mBadgeTitleColor == AUTO_COLOR ? model.getColor() :
                        mBadgeTitleColor);
            mBadgePaint.setAlpha((int) (MAX_ALPHA * model.mBadgeFraction));
    ...
    private float clampValue(final float value) {
        return Math.max(Math.min(value, NavigationTabBar.MAX_FRACTION), NavigationTabBar
                .MIN_FRACTION);
    }
